...
<script>
    ...
    function setDataRow(item, number) {
        ...
        row.appendChild(xiangmuNameCell);
            ...
            //项目预览图
        var xiangmuPhotoCell = document.createElement('td');
        var xiangmuPhotoImg = document.createElement('img');
        var xiangmuPhotoImgValue = item.xiangmuPhoto;
        if(xiangmuPhotoImgValue !=null && xiangmuPhotoImgValue !='' && xiangmuPhotoImgValue !='null'){
            xiangmuPhotoImg.setAttribute('src', xiangmuPhotoImgValue);
            xiangmuPhotoImg.setAttribute('height', 100);
            xiangmuPhotoImg.setAttribute('width', 100);
            xiangmuPhotoCell.appendChild(xiangmuPhotoImg);
        }else{
            xiangmuPhotoCell.innerHTML = "暂无图片";
        }
        row.appendChild(xiangmuPhotoCell);
        ...
        if(item.shifouTypes == 2){
            var detailBtn = document.createElement('button');
            var detailAttr = "shenbao(" + item.id + ')';
            detailBtn.setAttribute("type", "button");
            detailBtn.setAttribute("class", "btn btn-success btn-sm 申报");
            detailBtn.setAttribute("onclick", detailAttr);
            detailBtn.innerHTML = "申报"
            btnGroup.appendChild(detailBtn)
        }
    ...
    // 申报
    function shenbao(id) {
        var mymessage = confirm("真的要申报吗？");
        if (mymessage == true) {
            http("xiangmu/shenbao?ids="+id, "get", {}, (res) => {
                if(res.code == 0){
                    getDataList();
                    alert('申报成功，请等待专家审批');
                }
