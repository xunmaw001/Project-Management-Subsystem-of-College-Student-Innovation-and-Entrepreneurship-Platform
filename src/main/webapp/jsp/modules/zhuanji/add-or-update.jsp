                            ...
                            <!-- 当前表的字段 -->
                                ...
                                <div class="form-group col-md-6">
                                    ...
                                    <input id="zhuanjiIdNumber" name="zhuanjiIdNumber" class="form-control"
                                           onchange="zhuanjiIdNumberChickValue(this)"  placeholder="身份证号">
                                </div>
                                <div class="form-group col-md-6">
                                    <label>手机号</label>
                                    <input id="zhuanjiPhone" name="zhuanjiPhone" class="form-control"
                                           onchange="zhuanjiPhoneChickValue(this)"  placeholder="手机号">
                                </div>
                                   <div class="form-group col-md-6">
                                       <label>照片</label>
                                       <img id="zhuanjiPhotoImg" src="" width="100" height="100">
                                       <input name="file" type="file" id="zhuanjiPhotoupload"
                                              class="form-control-file">
                                       <input name="zhuanjiPhoto" id="zhuanjiPhoto-input" type="hidden">
                                   </div>
...
<script>
    ...
    function upload() {
        ...
        $('#zhuanjiPhotoupload').fileupload({
            url: baseUrl + 'file/upload',
            headers: {token: window.sessionStorage.getItem("token")},
            dataType: 'json',
            done: function (e, data) {
                var photoUrl= baseUrl + 'file/download?fileName=' + data.result.file;
                document.getElementById('zhuanjiPhotoImg').setAttribute('src',photoUrl);
                document.getElementById('zhuanjiPhoto-input').setAttribute('value',photoUrl);
            }
        });
    ...
    function submit() {
        if (validform() == true && compare() == true) {
            ...
            httpJson("zhuanji/" + urlParam, "POST", data, (res) => {
                if(res.code == 0){
                    window.sessionStorage.removeItem('addzhuanji');
    ...
    //数字检查
        <!-- 当前表的数字检查 -->
        function zhuanjiIdNumberChickValue(e){
        ...
        function zhuanjiPhoneChickValue(e){
            var this_val = e.value || 0;
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            rules: {
                username: "required",
                password: "required",
                zhuanjiName: "required",
                sexTypes: "required",
                zhuanjiIdNumber: "required",
                zhuanjiPhone: "required",
                zhuanjiPhoto: "required",
            },
            messages: {
                username: "账户不能为空",
                password: "密码不能为空",
                zhuanjiName: "专家姓名不能为空",
                sexTypes: "性别不能为空",
                zhuanjiIdNumber: "身份证号不能为空",
                zhuanjiPhone: "手机号不能为空",
                zhuanjiPhoto: "照片不能为空",
            }
        }).form();
    }
    ...
    // 获取当前详情
    function getDetails() {
        var addzhuanji = window.sessionStorage.getItem("addzhuanji");
        if (addzhuanji != null && addzhuanji != "" && addzhuanji != "null") {
            window.sessionStorage.removeItem('addzhuanji');
            //注册表单验证
            $(validform());
            $('#submitBtn').text('新增');
            ...
            window.sessionStorage.removeItem('updateId');
            http("zhuanji/info/" + updateId, "GET", {}, (res) => {
                if(res.code == 0)
                {
                    ruleForm = res.data
                    // 是/否下拉框回显
